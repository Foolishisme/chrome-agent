...
- 新增 `site_overview` task module，面向用户给出明确 URL，或明确要求查看某个官网/网站/站点的单站概况任务；普通公司产品、平台、文档、价格问题默认仍走 `public_research`，避免把多源问题误判成单站概况。
...
- `resolveEntryPoint` 负责显式 URL 直达校验，或在未给 URL 但明确官网/网站意图时做一次有界官网入口解析，并拒绝明显非官方候选。
