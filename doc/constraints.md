...
`debugger` 权限不是主要产品阻力，可以作为通用浏览器 Agent 的合理代价。
...
- 可优先评估：`debugger`、`tabs`、`scripting`、必要 host permissions。
- 按需评估：`offscreen`、`activeTab`、`storage`。
- 暂不作为第一阶段默认依赖：`identity`、`cookies`、`declarativeNetRequest`。

`<all_urls>` 可作为开发和通用能力验证阶段的现实选择，但产品化时必须重新评估说明、开关和权限裁剪。
