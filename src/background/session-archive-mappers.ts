...
import type { ArchivedConversation, ArchivedTurn, SaveSuccessfulSessionArchiveInput } from "./session-archive-types";
import { buildConversationTitle } from "./session-archive-store";
import { toConversationTurn } from "./session-archive-types";

function getAnswerMarkdownFromFinalResult(finalResult: SaveSuccessfulSessionArchiveInput["finalResult"]) {
  return finalResult.markdown || finalResult.artifacts.find((artifact) => artifact.kind === "markdown")?.content || finalResult.summary;
}

export function buildArchivedTurn(input: SaveSuccessfulSessionArchiveInput, turnId: number): ArchivedTurn {
