...
import type { SaveSuccessfulSessionArchiveInput } from "./session-archive-types";

export { createConversation, getPreferredConversation, toConversationTurns };

export async function saveSuccessfulSessionArchive(input: SaveSuccessfulSessionArchiveInput) {
  if (!input.sessionId || !input.goal || !input.finalResult || input.finalResult.status !== "success") {
    return undefined;
  }
